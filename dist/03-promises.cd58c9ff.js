...
const onSubmittingClick = (event)=>{
    ...
    let { delay: delay1 , step , amount  } = event.target.elements;
    delay1 = Number(delay1.value);
    step = Number(step.value);
    amount = Number(amount.value);
    if (amount <= 0 || delay1 < 0 || step < 0) {
        (0, _notiflixDefault.default).Notify.failure(` Please input correct values (>=0)`);
        return;
    }
    for(let position1 = 0; position1 < amount; position1 += 1){
...
},{}]},["dDjGg","8tzFL"], "8tzFL", "parcelRequire158a")
